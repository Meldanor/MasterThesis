...
Bases on the previous strategy the inspector can also utilize his knowledge about past episodes and prefer an inspection at time spans when a congestion happened in the past. This \textit{experience based} strategy yields the risk to find only a local optimum instead of a global one. The risk to get stuck in such a local optimum can be reduced by limiting the length of the history to give new inputs a chance to be recognized. 

The next parts will describe the changes from a general network to SCION itself.
