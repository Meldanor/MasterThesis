...
The exploration phase itself needs to be constantly repeated to react to changes inside the network. Inactive or disconnected nodes doesn't need to be monitored and can be removed from the graph. This reduces the graph which improves the performance impact by using fewer memory and fewer CPU cycles to iterate over.
