...
One solution to counter the predictability is to keep the algorithm and its configuration secret from the inspectees. The solution has two main flaws: The first one is, that \textit{security-by-obscurity} doesn't work on the long term. \todo{Quelle für beispiele}. Without going into detail about security problems, there is one weak spot in the security chain, the human itself. A dissatisfied employee can publish the algorithm and its configuration to the public as an act of revenge, sell the information or unintentionally leak it. The other flaw is, that given a big enough sample rate, the inspectees can statistically reverse-engineer the algorithm. \todo{Quelle für solche angriffe}
...
The now following criteria are \textbf{dynamic} and require a history of the past episodes. The amount of hold episodes is limited by the size of the network, because is needs memory per node. As seen later, the length of saved history will also have an impact on the quality of the selection process. \todo{Ändern, weil es nur noch eines ist}
...
\subsection{Conclusion} \label{sec:main:conclusionphase} \todo{Umbennen in Decision -> Auch Graphiken ändern}
...
\begin{figure}
	\centering
	\begin{tikzpicture}
	\tikzstyle{AS}=[circle,draw=black,scale=1]
	\begin{scope}
	\node[AS] (1-11) at (0,0)   {1-11};
	\node[AS] (1-12) at (4,0)   {1-12};
	\node[AS,draw=blue] (1-13) at (5,-2)  {1-13};
	\node[AS] (1-14) at (5,2)   {1-14};
	\end{scope}
	
	\path[<-]    
	(1-11) edge[draw=red] node[anchor=center, above] {\small 1.1/1.0 GB/s} (1-12) 
	(1-12) edge node[anchor=center, right] {\small 0.9/1.0 GB/s} (1-13)
	(1-12) edge node[anchor=center, right] {\small 0.2/1.0 GB/s} (1-14)
	;
	\end{tikzpicture}
...
\begin{figure}
	\centering
	\includegraphics[height=0.2\textheight]{overuseTolerance.pdf}
...
\begin{figure}
	\centering
	\includegraphics[height=0.2\textheight]{traffic_FRA-2days-1170-400}
